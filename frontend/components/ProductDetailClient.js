...
export default function ProductDetailClient({ product }) {
  ...
  return (
    <div className="container mx-auto px-4 py-16">
      <Link href="/products" className="text-sm font-bold text-gray-400 hover:text-gold uppercase tracking-wider mb-8 inline-block transition">
        ← Back to Shop
      </Link>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-16 mt-4">
        {/* Product Image */}
        <div className="glass-panel overflow-hidden flex items-center justify-center border border-white/5 rounded-2xl shadow-lg h-[500px] bg-[#111113]/40">
          <img 
            src={productImage} 
            alt={product.name} 
          ...
          {product.countInStock > 0 ? (
            <>
                  ...
                  <div className="flex space-x-3">
                    {product.sizes.map((size) => (
                      <button
                        ...
                        className={`w-12 h-12 border font-bold text-sm flex items-center justify-center uppercase transition duration-300 cursor-pointer rounded-lg ${
                          selectedSize === size
                            ? 'bg-gold text-black border-gold shadow-md'
                            : 'border-white/10 text-gray-400 hover:border-gold hover:text-gold bg-black/20'
                        }`}
                      >
                        {size}
                      </button>
              ...
              <div className="mb-8">
                ...
                <div className="flex items-center space-x-4">
                  <button 
                    ...
                    className="w-10 h-10 border border-white/10 text-gray-400 hover:border-gold hover:text-gold hover:bg-white/5 font-bold flex items-center justify-center disabled:opacity-30 cursor-pointer rounded-lg transition"
                  >
                    -
                  </button>
              ...
              <button
                ...
                className="btn-gold py-4 px-8 rounded-full text-sm font-extrabold uppercase tracking-widest shadow-md transition w-full md:w-auto cursor-pointer"
              >
                Add to Cart
              </button>
              ...
              {addedMessage && (
                <div className="mt-4 p-4 bg-green-950/40 border border-green-900/40 text-green-400 rounded-xl flex items-center">
            ...
            <div className="p-4 bg-red-950/40 border border-red-900/40 text-red-400 rounded-xl font-bold uppercase tracking-wider text-center">
              Out of Stock
            </div>
          )}
        </div>
